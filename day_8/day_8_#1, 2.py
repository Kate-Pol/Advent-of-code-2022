...
grid = np.array([list(x.strip()) for x in open('day_8.txt')], int)

#get number of rows and columns
nrow, ncol = np.shape(grid)

best_view_score = 1  # initialising the best view score (at least one form any tree)

n_tree_visible = ncol * 2 + (nrow - 2) * 2 # initialising visible trees (trees on the edge)
for ii in range(1, nrow - 1):  #go through all rows inside
    for iii in range(1, ncol - 1): # go through all the columns inside
        ...
        for tt in range(len(tree_up)):
            ...
            if tree_up[len(tree_up) - 1 - tt] >= tree:  # if we get to the tree that is the same hight or taller we break
                break
        ...
        for tt in range(len(tree_down)):
            ...
            if tree_down[tt] >= tree:  # if we get to the tree that is the same hight or taller we break
                break
        ...
        for tt in range(len(tree_right)):
            ...
            if tree_right[tt] >= tree:  # if we get to the tree that is the same hight or taller we break
                break
        ...
        for tt in range(len(tree_left)):
            ...
            if tree_left[len(tree_left) - 1 - tt] >= tree:  # if we get to the tree that is the same hight or taller we break
                break
                             
                
        
        # if tree is taller than the maxes in all directions we van see the tree
        if tree > tallest_tree_up or tree > tallest_tree_down or tree > tallest_tree_right or tree > tallest_tree_left:
            n_tree_visible += 1
            
        # if the new score view is higher we overwrite the best score
...
print('Part 2 - the best view score is: ' + str(best_view_score))
